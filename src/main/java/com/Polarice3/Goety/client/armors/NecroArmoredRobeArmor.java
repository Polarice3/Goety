        ...
        if (MainConfig.SoulRepair.get()) {
            ...
            if (stack.isDamaged()){
                if (SEHelper.getSEActive(player)){
                    if (SEHelper.getSESouls(player) > MainConfig.NecroArmoredRobeRepairAmount.get()){
                        stack.getTag().putInt(COOL, stack.getTag().getInt(COOL) + 1);
                        if (stack.getTag().getInt(COOL) > 20) {
                            stack.getTag().putInt(COOL, 0);
                            SEHelper.decreaseSESouls(player, MainConfig.NecroArmoredRobeRepairAmount.get());
                            stack.setDamageValue(stack.getDamageValue() - 1);
                        }
                    }
                } else if (!foundStack.isEmpty() && GoldTotemItem.currentSouls(foundStack) > MainConfig.NecroArmoredRobeRepairAmount.get()) {
                    stack.getTag().putInt(COOL, stack.getTag().getInt(COOL) + 1);
                    if (stack.getTag().getInt(COOL) > 20) {
                        stack.getTag().putInt(COOL, 0);
                        GoldTotemItem.decreaseSouls(foundStack, MainConfig.NecroArmoredRobeRepairAmount.get());
                        stack.setDamageValue(stack.getDamageValue() - 1);
                    }
                }
            }
