...
public class BlockMossRuinProcessor extends StructureProcessor {
    public static final Codec<BlockMossRuinProcessor> CODEC = Codec.FLOAT.fieldOf("mossiness").xmap(BlockMossRuinProcessor::new, (p_237064_0_) -> {
        return p_237064_0_.mossiness;
    }).codec();
    private final float mossiness;

    public BlockMossRuinProcessor(float p_i232115_1_) {
    ...
    private BlockState maybeReplaceFullStoneBlock(Random pRandom) {
        if (pRandom.nextFloat() >= 0.5F) {
            ...
            if (pRandom.nextBoolean()){
                ablockstate = new BlockState[]{Blocks.MOSSY_COBBLESTONE.defaultBlockState(), getRandomFacingStairs(pRandom, Blocks.MOSSY_COBBLESTONE_STAIRS)};
                ablockstate1 = new BlockState[]{Blocks.COBBLESTONE.defaultBlockState(), getRandomFacingStairs(pRandom, Blocks.COBBLESTONE_STAIRS)};
            }
    ...
    private BlockState maybeReplaceStairs(Random pRandom, BlockState pState) {
        ...
        if (pRandom.nextFloat() >= 0.5F) {
            ...
            if (pRandom.nextBoolean()){
                ablockstate = new BlockState[]{Blocks.COBBLESTONE_SLAB.defaultBlockState(), Blocks.STONE_BRICK_SLAB.defaultBlockState()};
                ablockstate1 = new BlockState[]{Blocks.MOSSY_COBBLESTONE_STAIRS.defaultBlockState().setValue(StairsBlock.FACING, direction).setValue(StairsBlock.HALF, half), Blocks.MOSSY_COBBLESTONE_SLAB.defaultBlockState()};
            }
            return this.getRandomBlock(pRandom, ablockstate, ablockstate1);
        }
    }
    ...
    private BlockState maybeReplaceSlab(Random pRandom) {
        if (pRandom.nextFloat() < this.mossiness){
            if (pRandom.nextBoolean()){
                return Blocks.MOSSY_STONE_BRICK_SLAB.defaultBlockState();
            } else {
                if (pRandom.nextInt(2) == 0){
                    return Blocks.MOSSY_COBBLESTONE_SLAB.defaultBlockState();
                } else {
