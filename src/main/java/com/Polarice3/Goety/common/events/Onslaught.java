...
public class Onslaught {
    ...
    private Onslaught.Status status;
    private int celebrationTicks;
    private Optional<BlockPos> waveSpawnPos = Optional.empty();

    public Onslaught(int p_i50144_1_, ServerWorld p_i50144_2_, PlayerEntity player) {
        this.id = p_i50144_1_;
        ...
        this.status = Onslaught.Status.ONGOING;
    }

    public Onslaught(ServerWorld p_i50145_1_, CompoundNBT p_i50145_2_) {
        ...
        this.targetPlayer = p_i50145_1_.getPlayerByUUID(p_i50145_2_.getUUID("TargetPlayer"));
        this.numGroups = p_i50145_2_.getInt("NumGroups");
        this.status = Onslaught.Status.getByName(p_i50145_2_.getString("Status"));
    ...
    public boolean isStopped() {
        return this.status == Onslaught.Status.STOPPED;
    }

    public boolean isVictory() {
        return this.status == Onslaught.Status.VICTORY;
    }

    public boolean isLoss() {
        return this.status == Onslaught.Status.LOSS;
    }
    ...
    public void stop() {
        ...
        this.status = Onslaught.Status.STOPPED;
    }

    public void tick() {
        if (!this.isStopped()) {
            if (this.status == Onslaught.Status.ONGOING) {
                boolean flag = this.active;
                this.active = this.level.hasChunkAt(this.targetPlayer.blockPosition());
                ...
                if (!this.targetPlayer.isAlive()) {
                    if (this.groupsSpawned > 0) {
                        this.status = Onslaught.Status.LOSS;
                    } else {
                        this.stop();
                    }
                }
                ...
                if (this.isStarted() && !this.hasMoreWaves() && i == 0) {
                    if (this.postRaidTicks < 40) {
                        ...
                        this.status = Onslaught.Status.VICTORY;

                        IInfamy infamy = InfamyHelper.getCapability(this.targetPlayer);
                        infamy.setInfamy(0);
                        ...
                        for(UUID uuid : this.heroesOfTheVillage) {
                            ...
                            if (entity instanceof LivingEntity && !entity.isSpectator()) {
                                ...
                                livingentity.addEffect(new EffectInstance(Effects.HERO_OF_THE_VILLAGE, 48000, 4, false, false, true));
    ...
    private void updateRaiders() {
        ...
        while(iterator.hasNext()) {
            ...
            for(AbstractRaiderEntity abstractraiderentity : set1) {
                ...
                if (!abstractraiderentity.removed && abstractraiderentity.level.dimension() == this.level.dimension() && !(this.targetPlayer.blockPosition().distSqr(blockpos) >= 12544.0D)) {
    ...
    private void spawnGroup(BlockPos p_221294_1_) {
        ...
        for(Onslaught.WaveMember raid$wavemember : Onslaught.WaveMember.VALUES) {
    ...
    public void joinRaid(int pWave, AbstractRaiderEntity p_221317_2_, @Nullable BlockPos p_221317_3_, boolean p_221317_4_) {
        boolean flag = this.addWaveMob(pWave, p_221317_2_);
        if (flag) {
            if (!p_221317_4_ && p_221317_3_ != null) {
                ...
                p_221317_2_.applyRaidBuffs(pWave, false);
                p_221317_2_.goalSelector.addGoal(0, new MoveTowardsTargetGoal<>(p_221317_2_));
    ...
    private BlockPos findRandomSpawnPos(int p_221298_1_, int p_221298_2_) {
        ...
        for(int i1 = 0; i1 < p_221298_2_; ++i1) {
            ...
            int j = this.targetPlayer.blockPosition().getX() + MathHelper.floor(MathHelper.cos(f) * 32.0F * (float)i) + this.level.random.nextInt(5);
            int l = this.targetPlayer.blockPosition().getZ() + MathHelper.floor(MathHelper.sin(f) * 32.0F * (float)i) + this.level.random.nextInt(5);
    ...
    public void setLeader(int pRaidId, AbstractRaiderEntity p_221324_2_) {
        this.groupToLeaderMap.put(pRaidId, p_221324_2_);
    ...
    private int getDefaultNumSpawns(Onslaught.WaveMember p_221330_1_, int p_221330_2_, boolean p_221330_3_) {
        return p_221330_3_ ? p_221330_1_.spawnsPerWaveBeforeBonus[this.numGroups] : p_221330_1_.spawnsPerWaveBeforeBonus[p_221330_2_];
    }

    private int getPotentialBonusSpawns(Onslaught.WaveMember p_221335_1_, Random p_221335_2_, int pWave, DifficultyInstance p_221335_4_, boolean p_221335_5_) {
    ...
    public CompoundNBT save(CompoundNBT pNbt) {
        ...
        pNbt.putUUID("TargetPlayer", this.targetPlayer.getUUID());
    ...
    static enum Status {
        ...
        private static final Onslaught.Status[] VALUES = values();

        private static Onslaught.Status getByName(String pName) {
            for(Onslaught.Status raid$status : VALUES) {
    ...
    public static enum WaveMember implements net.minecraftforge.common.IExtensibleEnum {
        VINDICATOR(EntityType.VINDICATOR, new int[]{1, 2, 2, 3, 4, 4, 4, 5}),
        EVOKER(EntityType.EVOKER, new int[]{1, 1, 0, 1, 2, 2, 3, 4}),
        PILLAGER(EntityType.PILLAGER, new int[]{4, 4, 3, 3, 4, 4, 4, 6}),
        WITCH(EntityType.WITCH, new int[]{1, 1, 0, 3, 3, 0, 0, 0}),
        BELDAM(ModEntityType.BELDAM.get(), new int[]{0, 0, 0, 0, 0, 1, 1, 2}),
        RAVAGER(EntityType.RAVAGER, new int[]{0, 1, 0, 1, 1, 1, 2, 4}),
        ...
        private static Onslaught.WaveMember[] VALUES = values();
        ...
        public static Onslaught.WaveMember create(String name, EntityType<? extends AbstractRaiderEntity> typeIn, int[] waveCountsIn) {
