...
import com.Polarice3.Goety.common.blocks.tiles.CursedKilnTileEntity;
...
public class CursedKilnBlock extends ContainerBlock implements IWaterLoggable {
    ...
    public ActionResultType use(BlockState pState, World pLevel, BlockPos pPos, PlayerEntity pPlayer, Hand pHand, BlockRayTraceResult pHit) {
        ...
        if (tileentity instanceof CursedKilnTileEntity) {
            CursedKilnTileEntity kilnTileEntity = (CursedKilnTileEntity)tileentity;
            ItemStack itemstack = pPlayer.getItemInHand(pHand);
            Optional<FurnaceRecipe> optional = kilnTileEntity.getRecipes(itemstack);
            if (optional.isPresent()) {
                if (!pLevel.isClientSide && kilnTileEntity.placeItem(pPlayer.abilities.instabuild ? itemstack.copy() : itemstack, optional.get().getCookingTime())) {
            ...
            if (itemstack.isEmpty()){
                if (!kilnTileEntity.getItems().isEmpty()){
                    InventoryHelper.dropContents(pLevel, pPlayer.blockPosition(), kilnTileEntity.getItems());
                    kilnTileEntity.getItems().clear();
                    return ActionResultType.SUCCESS;
                }
                return ActionResultType.CONSUME;
            }
    ...
    public void onRemove(BlockState pState, World pLevel, BlockPos pPos, BlockState pNewState, boolean pIsMoving) {
        if (!pState.is(pNewState.getBlock())) {
            TileEntity tileentity = pLevel.getBlockEntity(pPos);
            if (tileentity instanceof CursedKilnTileEntity) {
                InventoryHelper.dropContents(pLevel, pPos, ((CursedKilnTileEntity)tileentity).getItems());
            }
    ...
    public TileEntity newBlockEntity(IBlockReader p_196283_1_) {
        return new CursedKilnTileEntity();
    }
