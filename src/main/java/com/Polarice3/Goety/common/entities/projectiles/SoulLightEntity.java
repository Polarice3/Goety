...
public class SoulLightEntity extends ArrowEntity {
    ...
    public void tick() {
        ...
        if(level.isClientSide) {
            ...
            this.level.addParticle(ModParticleTypes.SOUL_LIGHT_EFFECT.get(), this.getX(), this.getY(), this.getZ(), 0.0F, 0.0F, 0.0F);
            this.level.addParticle(ModParticleTypes.BULLET_EFFECT.get(), d0 + level.random.nextDouble()/2, d1 + 0.5D, d2 + level.random.nextDouble()/2, 0.0D, 0.0D, 0.0D);
        }
    }
    ...
    protected void onHitBlock(BlockRayTraceResult pResult) {
        ...
        if (this.getOwner() != null) {
            if (this.level.getBlockState(pos).getMaterial().isReplaceable() && this.level.isUnobstructed(ModBlocks.SOUL_LIGHT_BLOCK.get().defaultBlockState(), pos, ISelectionContext.of(this.getOwner()))) {
                this.level.setBlockAndUpdate(pos, ModBlocks.SOUL_LIGHT_BLOCK.get().defaultBlockState());
            }
        }
        this.remove();
    }
