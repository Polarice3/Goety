...
public class DredenEntity extends MonsterEntity implements ISpewing, IRangedAttackMob {
    protected static final DataParameter<Boolean> SPEWING = EntityDataManager.defineId(DredenEntity.class, DataSerializers.BOOLEAN);

    public DredenEntity(EntityType<? extends MonsterEntity> p_i48553_1_, World p_i48553_2_) {
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(4, new FrostBallGoal(this));
    ...
    public boolean isAlliedTo(Entity entityIn) {
        if (entityIn instanceof StrayEntity) {
    ...
    public boolean canBeAffected(EffectInstance potioneffectIn) {
        return potioneffectIn.getEffect() != ModEffects.HOSTED.get() && potioneffectIn.getEffect() != Effects.MOVEMENT_SLOWDOWN;
    }
    ...
    class MoveRandomGoal extends Goal {
        ...
        public boolean canUse() {
            return !DredenEntity.this.getMoveControl().hasWanted()
                    && DredenEntity.this.isNoGravity()
                    && DredenEntity.this.random.nextInt(7) == 0;
        }
        ...
        public void tick() {
            BlockPos blockpos = DredenEntity.this.blockPosition();

            for(int i = 0; i < 3; ++i) {
                BlockPos blockpos1 = blockpos.offset(DredenEntity.this.random.nextInt(15) - 7, DredenEntity.this.random.nextInt(11) - 5, DredenEntity.this.random.nextInt(15) - 7);
                if (DredenEntity.this.level.getFluidState(blockpos1).isSource() || DredenEntity.this.level.isEmptyBlock(blockpos1)) {
                    DredenEntity.this.moveControl.setWantedPosition((double)blockpos1.getX() + 0.5D, (double)blockpos1.getY() + 0.5D, (double)blockpos1.getZ() + 0.5D, 0.25D);
                    if (DredenEntity.this.getTarget() == null) {
                        DredenEntity.this.getLookControl().setLookAt((double)blockpos1.getX() + 0.5D, (double)blockpos1.getY() + 0.5D, (double)blockpos1.getZ() + 0.5D, 180.0F, 20.0F);
                    }
                    break;
                }
            }
    ...
    static class FrostBallGoal extends RangedAttackGoal{
        public DredenEntity ghost;

        public FrostBallGoal(DredenEntity entity) {
